...
- PWAの追加は端末ごとに一度だけです。イベントごとの再インストールは不要で、アプリ内の「共同編集」から8文字の共有コードを入力してイベントを切り替えられます。
...
### 使い方
1. ヘッダーの「共同編集」→「共有を開始してコードを作成」を押すと、今の画面の内容でルームが作られ、8文字の共有コードが発行される（`VITE_ROOM_PASSWORD` 設定時はここで合言葉の入力を求められる）。
2. 参加者は同じPWAを開き、「共同編集」から共有コードを入力する。従来どおり共有URLを開いて参加することもできる。
3. 参加中は、他の人のマウスカーソルとニックネームが画面上に表示され、誰かがバンドカードをドラッグ中はそのカードが他の人には「🔒 ○○が移動中」とロック表示され、自分ではドラッグできなくなる。

## 当日のステージ進行

- タイムテーブル上部の「進行モードを開く」で、現在・次・その次と運営リモコンを表示します。
- 「開始」は現在時刻との差を計算し、「終了」は転換へ、「転換完了」は次の枠へ進めます。`+1分`・`+5分`・`定刻へ戻す`は選択位置以降へ波及し、確定前に出演者・変更時刻・新しい連続出演・終演予定を確認できます。
- 共同編集メニューの「QR共有」から共同編集、閲覧専用パンフレット、会場スクリーンのQRを表示できます。
- パンフレットでバンドを選ぶと出演者マイページになり、出演・集合・リハーサル・前後予定・通知を確認できます。`?mode=screen` は操作ボタンのない会場全画面表示です。
- `Aa` から文字拡大、高コントラスト、Reduce Motion、タップ領域拡大を切り替えられます。

進行状態の公開同期には `publicProgress/{roomId}` を使います。この機能を公開環境で有効にする際は、更新済みの `firestore.rules` もFirebaseコンソールへ反映してください。
