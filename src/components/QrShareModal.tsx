...
export function QrShareModal({ roomId, onClose }: { roomId: string; onClose: () => void }) {
  ...
  function ShareCard({ label, code, url, explanation, prominent = false }: ShareCardProps) {
    return (
      <section className={`flex min-w-0 flex-col items-center rounded-lg border p-3 ${prominent ? "border-blue-500 bg-blue-950/30" : "border-slate-700"}`}>
        ...
        {code && (
          <div className="mt-3 flex w-full min-w-0 items-center gap-2 rounded-lg border border-slate-700 bg-slate-950 p-2">
            ...
            <button type="button" onClick={() => void copy(code.toUpperCase(), `${label}コード`)} className="min-h-11 shrink-0 rounded-lg border border-slate-600 px-3 text-xs font-semibold text-slate-300 hover:bg-slate-700">コードをコピー</button>
          </div>
        )}
        ...
        <button type="button" onClick={() => void copy(url, `${label}URL`)} className={`mt-2 min-h-11 shrink-0 rounded-lg px-3 text-xs font-semibold ${prominent ? "bg-blue-600 text-white hover:bg-blue-500" : "border border-slate-600 text-slate-300 hover:bg-slate-700"}`}>URLをコピー</button>
      </section>
    );
  }
          ...
          <div className="w-full max-w-4xl rounded-xl border border-slate-700 bg-slate-900 p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="mx-auto mt-4 max-w-md">
              {viewerUrl && viewerCode ? (
                <ShareCard label="一般閲覧用コード" code={viewerCode} url={viewerUrl} explanation="一般部員・出演者・来場者にはこちらを共有してください。編集はできません。" prominent />
              ) : (
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-3">
              <ShareCard label="運営スタッフ用コード" code={roomId} url={`${base}?room=${roomId}`} explanation="運営スタッフ専用です。一般部員には共有しないでください。" />
              <ShareCard label="PA／ローディー用" url={`${base}pa-viewer?room=${roomId}`} />
              <ShareCard label="会場スクリーン" url={viewerCode ? `${base}${viewerCode}/public?mode=screen` : `${base}${roomId}/public?mode=screen`} />
            </div>
          </div>
        </div>
      </div>
