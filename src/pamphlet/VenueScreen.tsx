...
export function VenueScreen({ data, progress, circleId }: { data: PublicPamphletDoc; progress: StageProgress | null; circleId: string }) {
  ...
  return <main className="flex min-h-screen flex-col bg-[#020617] p-[4vw] text-white" aria-live="polite"><header className="flex items-start justify-between"><div><p className="text-[clamp(1rem,2vw,2rem)] font-semibold tracking-[0.2em] text-blue-300">LIVE TIMETABLE</p><h1 className="mt-2 text-[clamp(2rem,5vw,5rem)] font-black leading-none">{data.liveName || "Stage"}</h1></div><div className="rounded-2xl bg-white p-2"><QrCode value={publicUrl} label="観客向けパンフレット" size={140} /></div></header><section className="flex flex-1 flex-col justify-center"><p className="text-[clamp(1.3rem,3vw,3rem)] font-bold text-blue-300">{phase === "transition" ? "転換中" : phase === "break" ? "休憩中" : phase === "finished" ? "本日の公演は終了しました" : current ? "現在出演中" : `開演まで ${countdownText}`}</p><h2 className="mt-4 text-[clamp(3rem,10vw,10rem)] font-black leading-[0.95] tracking-tight">{phase === "transition" ? "STAGE CHANGE" : label(current)}</h2>{current && <p className="mt-5 font-mono text-[clamp(1.5rem,4vw,4rem)] text-slate-300">{current.slot.startTime} — {current.slot.endTime}</p>}</section><footer className="grid grid-cols-[auto_1fr] items-center gap-6 border-t border-slate-700 pt-5"><span className="text-[clamp(1rem,2vw,2rem)] font-bold text-slate-400">NEXT</span><strong className="truncate text-[clamp(2rem,5vw,5rem)]">{label(next)}</strong></footer></main>;
}
